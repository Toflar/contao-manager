<template>
    <boot-check :progress="bootState" :title="$t('ui.server.contao.title')" :description="bootDescription">
        <button class="widget-button widget-button--primary widget-button--run" :disabled="!isGranted(scopes.INSTALL)" @click="setup" v-if="bootState === 'action'">
            {{ $t(`ui.server.contao.${databaseProblem && !databaseAccessProblem ? 'check' : 'setup'}`) }}
        </button>
        <button class="widget-button widget-button--alert" :disabled="!isGranted(scopes.INSTALL)" @click="setup" v-if="bootState === 'warning'">
            {{ $t(`ui.server.contao.${databaseProblem && !databaseAccessProblem ? 'check' : 'setup'}`) }}
        </button>
        <button class="widget-button" :disabled="!isGranted(scopes.INSTALL)" @click="setup" v-if="bootState === 'success' && databaseSupported">
        ...
        async boot() {
            ...
            if (response.status === 200) {
                if (!result.version) {
                    ...
                    bootState = 'success';
                    bootDescription = this.$t('ui.server.contao.found', {
                        version: result.version,
                        api: result.api.version,
                    });
                }
