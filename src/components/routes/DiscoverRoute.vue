<template>
    <discover-list :wrapper="wrapper" hide-themes>
        <template #package-hint="{ data }">
            <p v-if="!contaoSupported(data.contaoConstraint)">
        ...
        <template #package-actions="{ data }">
            <a
                class="widget-button widget-button--small widget-button--primary widget-button--link"
                target="_blank"
                :href="data.homepage"
                v-if="data && data.private && !packageSuggested(data.name)"
                >{{ $t('ui.package.homepage') }}</a
            >
